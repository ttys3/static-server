...
async fn main() {
    ...
    let app = Router::with_state(StaticServerConfig { root_dir })
        ...
        .layer(TraceLayer::new_for_http().make_span_with(|request: &Request<Body>| {
            ...
            tracing::debug_span!("client-addr", addr = %addr)
        }));
...
async fn video_frame_thumbnail(State(cfg): State<StaticServerConfig>, Query(params): Query<HashMap<String, String>>) -> impl IntoResponse {
    ...
    let default_thumbnail = base64::decode(video_icon).unwrap();
    ...
    match child.wait_with_output().await {
        Ok(out) => {
            ...
            if out.status.success() {
                ...
                tracing::error!("video_frame_thumbnail failed, code={:?} stderr={} stdout={}", out.status.code(), String::from_utf8_lossy(&stderr), String::from_utf8_lossy(&stdout));
